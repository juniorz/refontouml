...
public class FileManager
{
	...
	public FileManager (File sourceFile)
	{
		...
		try
		{
			...
			// Create a folder
			File folder = new File(f1.getParent() + File.separatorChar + name + " Documentation" + File.separatorChar);
			folder.mkdir();
	...
	private void CopyImage (String path, String imgname) throws IOException
	{
		...
		URL fakeUrl = b.getEntry("/img/" + imgname);
		URL url = FileLocator.toFileURL(fakeUrl);
		File myimg = new File(url.getPath());
		
		if (myimg.exists())
		{
			File myimgcpy = new File(path + File.separatorChar + imgname);
			
			InputStream in = new FileInputStream(myimg);
